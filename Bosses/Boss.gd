...
export (PackedScene) var Rain_Bullet
...
func rain_shoot():
	for tiro in rain_list:
		var rain_bullet_instance = Rain_Bullet.instance()
		emit_signal("boss_shooted", rain_bullet_instance, tiro.global_position, Vector2.DOWN)
